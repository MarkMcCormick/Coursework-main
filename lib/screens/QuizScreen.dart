...
class QuizScreenState extends State<QuizScreen> {
  ...
  nextQuestion(Qdata){
    ...
    setState(() {
      ...
      globals.currentQuestionSL = Qdata.sublist(0)[intValue]['skill_level'];
      List CQAns = [];
  ...
  isCorrect(AB){
    ...
    if (AB == true) {
      ...
      globals.rgb2 = 250;
      globals.rgb3 = 77;
      globals.CScore += 10 * globals.currentQuestionSL;
      globals.JScore = 10 * globals.currentQuestionSL;
      globals.TScore += 10 * globals.currentQuestionSL;
      globals.CStreak += 1;
      if (globals.CStreak == 10){
        globals.CStreak = 0;
        globals.LevelUp = true;
      }
    } else {
  ...
  Future<List<Map<String, dynamic>>> QuizQuery() async {
    ...
    if (globals.LevelUp == true){
      final update = await conn.execute(
        Sql.named('UPDATE player SET skill_level = @SKL WHERE username = @user'),
        parameters: {'user': "user1" , "SKL" : (int.parse(skillLevel[0][0].toString()) + 1)},
      );
    }

    final TotalScore = await conn.execute(
      // Fetch User level
        "SELECT score FROM player WHERE username = 'user1'" //change this for the logged in user
    );
    globals.TScore = int.parse(TotalScore[0][0].toString());
    print(int.parse(TotalScore[0][0].toString()));
    print(TotalScore[0][0].toString());
    print(globals.TScore);
    print("German");
    print(TotalScore);
    print(TotalScore[0][0]);
    print(globals.TScore.toString());
    print(TotalScore[0][0].toString());
    if (int.parse(globals.TScore.toString()) >= int.parse(TotalScore[0][0].toString())){
      globals.TScore = int.parse(TotalScore[0][0].toString()) + globals.JScore;
      print("France");
      final update = await conn.execute(
        Sql.named('UPDATE player SET score = @score WHERE username = @user'),
        parameters: {'user': "user1" , "score" : globals.TScore},
      );
    }
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        child: Center(
                          child: Text(" Current Score: " + globals.CScore.toString() + " ",
                      ...
                      Container(
                        ...
                        child: Center(
                          child: Text(" Total Score: " + globals.TScore.toString() + " ", //add user total score here
                              style: TextStyle(color: Colors.black, fontSize: 20),
                              textAlign: TextAlign.center), //question display text
                        ),
                      ),
                  ...
                  Container(
                    ...
                    child: Center(
                      child: Text("Answer streak: " + globals.CStreak.toString() + "/10 ",
                          style: TextStyle(color: Colors.black, fontSize: 20),
                          textAlign: TextAlign.center), //question display text
                    ),
                  ),
